    ...
    @section('content')
        ...
        <div class="w-75 d-block mx-auto mt-3 p-4  ">
            <nav class="border rounded p-2 mb-3 shadow-sm">
                ...
                <a href="" class="text-body-tertiary">tambah</a>
            </nav>
            <form class="border rounded p-4 shadow-sm" method="POST" action="{{ route('admin.users.store') }}"> 
                @if(Session::get('error'))
                    <div class="alert alert-danger">
                        {{ Session('error') }}
                    </div>
                @endif
                <h5 class="text-center">Tambah Data Petugas</h5>
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="email" id="email" rows="5" class="form-control @error('email')is-invaid @enderror" value={{ old('email') }}>
                    @error('email')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="password" name="password" id="password" rows="5" class="form-control @error('password')is-invalid @enderror" >
                    @error('password')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
